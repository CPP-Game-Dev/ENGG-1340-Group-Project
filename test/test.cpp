...
char getTileChar(TileObject tile) {
    switch (tile) {
        ...
        case Wall: return '#';
        case None: return '.';
        case RegenSmall: return 'r';
        case RegenMedium: case RegenBig: return 'R';
        case StaminaSmall: return 's';
        case StaminaMedium: case StaminaBig: return 'S';
        case VisionSmall: return 'v';
        case VisionMedium: case VisionBig: return 'V';
        default: return '?';
    }
}
...
int getTileColor(TileObject tile) {
    switch (tile) {
        case Player: return 1; // Player as Yellow
        case Wall: return 2;   // Wall as White
        case None: return 3;   // None as Green
        default: return 4;     // Other tiles as Green
    }
}
int mapfov(int fov){
    if(fov == 0 || fov == 1){
...
void printMap(const vector<vector<TileObject>>& map, int playerX, int playerY, int fov) {
    for (int i = 0; i < (int) map.size(); i++) {
        for (int j = 0; j < (int) map[i].size(); j++) {
            if (i >= playerY - fov && i <= playerY + fov && j >= playerX - fov && j <= playerX + fov) {
                // Tile is within the field of view
                
                int tile_color = getTileColor(map[i][j]);
                attron(COLOR_PAIR(tile_color));
                
                if (map[i][j] == Wall || map[i][j] == Player || map[i][j] == None) {
                    printw("  "); // Print two spaces as a "block"
                } else {
                    char t = getTileChar(map[i][j]);
                    if (map[i][j] == RegenBig || map[i][j] == StaminaBig || map[i][j] == VisionBig) {
                        attron(A_BOLD);
                        printw("%c ", t);
                        attroff(A_BOLD);
                    } else {
...
int main() {
    TesterProgram();
    /*
        Gamemethod: printmap -> getch -> move(modification of tiles) ->get player pos -> printmap -> REPEAT
    */
    return 0;
}
